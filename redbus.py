...
#chandigarh bus
chandigarh=[]
df_C=pd.read_csv("df_Ch.csv")
for i,r in df_C.iterrows():  #traverse through each row
    chandigarh.append(r['Route_name'])   # add that row in new list
...
if menu=="Bus Routes":
    ...
    with col1:
        state=st.selectbox("List of States",["chandigarh", "Andhra Pradesh", "Telungana",  "Rajasthan", "Punjab",
    ...
    if state=="chandigarh":
        ...
        def type_and_fare(bus_type, fare_range,rate_range):
            conn=pymysql.connect(host="localhost",user="root",password="123456789",database="redbus",port=3306)
